...
- [`🌐` ︲ Documentation TP : Installer et configurer un service DHCP](#--documentation-tp--installer-et-configurer-un-service-dhcp)
  - [📑 ︲ Sommaire](#--sommaire)
- [`📘` ︲ Introduction](#--introduction)
  - [`❔` ︲ Contexte et objectifs du TP](#--contexte-et-objectifs-du-tp)
  - [`🧰` ︲ Présentation des outils et prérequis](#--présentation-des-outils-et-prérequis)
- [`🧩` ︲ Mission 1 : Choisir un plan d'adressage et définir l'adresse IP du serveur](#--mission-1--choisir-un-plan-dadressage-et-définir-ladresse-ip-du-serveur)
  - [`🎯` ︲ Objectif](#--objectif)
  - [`🛠️` ︲ Étape 1 : Étude du Contexte](#️--étape-1--étude-du-contexte)
    - [Équipements à prendre en compte](#équipements-à-prendre-en-compte)
    - [Choix du plan d'adressage](#choix-du-plan-dadressage)
    - [Nombre d'hôtes](#nombre-dhôtes)
  - [`🛠️` ︲ Étape 2 : Schéma Réseau](#️--étape-2--schéma-réseau)
  - [`🛠️` ︲ Étape 3 : Configuration de l'adresse IP statique du serveur](#️--étape-3--configuration-de-ladresse-ip-statique-du-serveur)
    - [`⚙️` ︲ Paramètres IP du serveur](#️--paramètres-ip-du-serveur)
    - [`📍` ︲ Accès à la configuration réseau](#--accès-à-la-configuration-réseau)
- [`🧩` ︲ Mission 2 : Installer le rôle DHCP](#--mission-2--installer-le-rôle-dhcp)
  - [`🎯` ︲ Objectif](#--objectif-1)
  - [`🛠️` ︲ Procédure d'installation](#️--procédure-dinstallation)
- [`🧩` ︲ Mission 3 : Créer une Étendue DHCP](#--mission-3--créer-une-étendue-dhcp)
  - [`🎯` ︲ Objectif](#--objectif-2)
  - [`🛠️` ︲ Procédure de création de l'étendue](#️--procédure-de-création-de-létendue)
  - [`⚙️` ︲ Paramètres de l'étendue](#️--paramètres-de-létendue)
  - [`🌍` ︲ Options DHCP](#--options-dhcp)
  - [`🔍` ︲ Configuration côté client](#--configuration-côté-client)
  - [`📡` ︲ Observation des échanges DHCP avec Wireshark](#--observation-des-échanges-dhcp-avec-wireshark)
    - [Lancer Wireshark sur le client](#lancer-wireshark-sur-le-client)
    - [Reproduire une libération/renouvellement](#reproduire-une-libérationrenouvellement)
    - [Appliquer le filtre DHCP](#appliquer-le-filtre-dhcp)
    - [Analyse des échanges DHCP](#analyse-des-échanges-dhcp)
- [`🧩` ︲ Mission 4 : Créer une Plage d'Exclusion](#--mission-4--créer-une-plage-dexclusion)
  - [`🎯` ︲ Objectif](#--objectif-3)
  - [`🛠️` ︲ Procédure](#️--procédure)
- [`🧩` ︲ Mission 5 : Créer des Réservations d'Adresses](#--mission-5--créer-des-réservations-dadresses)
  - [`🎯` ︲ Objectif](#--objectif-4)
  - [`🛠️` ︲ Procédure de création des réservations](#️--procédure-de-création-des-réservations)
  - [`🖨️` ︲ Tableau des Réservations](#️--tableau-des-réservations)
- [`🧩` ︲ Mission 6 : Créer un Basculement entre Deux Serveurs DHCP (Bonus)](#--mission-6--créer-un-basculement-entre-deux-serveurs-dhcp-bonus)
  - [`🎯` ︲ Objectif](#--objectif-5)
  - [`🛠️` ︲ Procédure](#️--procédure-1)
    - [Étape 1 : Cloner le serveur DHCP principal](#étape-1--cloner-le-serveur-dhcp-principal)
    - [Étape 2 : Modifier l'adresse IP du serveur secondaire](#étape-2--modifier-ladresse-ip-du-serveur-secondaire)
    - [Étape 3 : Configurer le basculement](#étape-3--configurer-le-basculement)
    - [Étape 4 : Vérification du basculement](#étape-4--vérification-du-basculement)
